...
public class AnalysisterTest {
    ...
    public void testStatistic() {
        ...
        WorkLogStandLine standLine = analysister.statistic("BTB-11221 测试项目 (2021-02-25 提测)");
        WorkLogStandLine expectedLine = new WorkLogStandLine();
        expectedLine.setDatetime("2021-02-25");
        expectedLine.setWorkInfo("BTB-11221测试项目");
        expectedLine.setSourceStr("BTB-11221 测试项目 (2021-02-25 提测)");
    ...
    public void testStatistic2() {
        String regx = "[（\\(]\\S*[1-9]{1,2}月\\d+[日号]\\S*[\\)）]|[（\\(]\\S*[01]{0,1}[1-9]{1,2}-[01]{0,1}[1-9]{1,2}\\S*[\\)）]";
        Analysister analysister = new Analysister(regx,"设计","提测","上线","发布");
        WorkLogStandLine standLine = analysister.statistic("BTB-11274  P0二类测试看看 （5月26号提测，6月1号上线）");
        WorkLogStandLine expectedLine = new WorkLogStandLine();
        expectedLine.setDatetime("5月26号");
        expectedLine.setWorkInfo("BTB-11274P0二类测试看看");
        expectedLine.setSourceStr("BTB-11274  P0二类测试看看 （5月26号提测，6月1号上线）");
        expectedLine.setFlag("提测");
        Assert.assertEquals(expectedLine, standLine);
    }
    ...
    public void testStatistic3() {
        String regx = "[（\\(]\\S*[1-9]{1,2}月\\d+[日号]\\S*[\\)）]|[（\\(]\\S*[01]{0,1}[1-9]{1,2}-[01]{0,1}[1-9]{1,2}\\S*[\\)）]";
        Analysister analysister = new Analysister(regx,"设计","提测","上线","发布");
        WorkLogStandLine standLine = analysister.statistic("BTB-11274  P0有效期判断逻辑统一 （配合提测，6月1号上线）");
        WorkLogStandLine expectedLine = new WorkLogStandLine();
        expectedLine.setDatetime("6月1号");
        expectedLine.setWorkInfo("BTB-11274P0有效期判断逻辑统一");
        expectedLine.setSourceStr("BTB-11274  P0有效期判断逻辑统一 （配合提测，6月1号上线）");
    ...
    public void testStatistic4() {
        String regx = "[（\\(]\\S*[1-9]{1,2}月\\d+[日号]\\S*[\\)）]|[（\\(]\\S*[01]{0,1}[1-9]{1,2}-[01]{0,1}[1-9]{1,2}\\S*[\\)）]";
        Analysister analysister = new Analysister(regx,"设计","提测","上线","发布");
        WorkLogStandLine standLine = analysister.statistic("1、BTB-11186 【技术】交易服务慢SQL优化第八期（2021-05-18 上线）");
        WorkLogStandLine expectedLine = new WorkLogStandLine();
        expectedLine.setDatetime("2021-05-18");
        expectedLine.setWorkInfo("TB-11186【技术】交易服务慢SQL优化第八期");
        expectedLine.setSourceStr("1、BTB-11186 【技术】交易服务慢SQL优化第八期（2021-05-18 上线）");
        expectedLine.setFlag("上线");
        Assert.assertEquals(expectedLine, standLine);
    }
}